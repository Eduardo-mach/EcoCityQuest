...
func inicializar_exercicio():
	...
	# Define a duração do exercício (exemplo: 3 minutos = 180 segundos)
	tempo_restante = 180
	_atualizar_label()

	# Configura o Timer para disparar a cada segundo continuamente
...
func _verificar_fim_do_exercicio():
	if telhado_concluido:
		...
		# Usa o TimerExercicio para esperar 3 segundos
		timer_exercicio.stop()
		timer_exercicio.wait_time = 3.0
		timer_exercicio.one_shot = true
		timer_exercicio.start()
